...
ADR 0004 records the full app's move to account-cloud-first capture and supersedes ADR 0002 and the WebRTC transport in ADR 0003. Its App Clip boundary describes an earlier phase; the current App Clip uses cloud delivery, as described below. Current access rules are documented in [authentication and billing](docs/authentication-and-billing.md). The full app is account-cloud-first: every accepted capture belongs to the signed-in Clerk user's Cloud Scanner Workspace, is recorded in a durable Local Capture Outbox before attempting delivery, synchronizes metadata through Convex, and transfers private photo bytes directly to Cloudflare R2 with short-lived presigned URLs. The full app bootstraps its Device Credential automatically from a Clerk bearer JWT after sign-in — no pairing, WebRTC connection, or Chrome-generated enrollment QR is required for capture or cloud sync. A selected Chrome computer is only an optional live cursor-insertion target reached through a per-result Convex delivery; it never owns a capture or a batch, and having no live target is a valid state.

The App Clip is a separate free, temporary cloud client. A QR from signed-in Chrome opens `/clip` with a short-lived workspace guest grant. The Clip sends captures through Convex/R2 and can choose among the workspace's online computers for text and barcode insertion. It has no WebRTC transport, Clerk session, durable Device Credential, or checkout.
...
- Pro Entitlement: an active, server-verified StoreKit subscription, including its introductory trial, or complimentary workplace/admin access. Pro enables cloud workspace and mobile capture sync. Signed-in users can open local iPhone capture and history without Pro.
...
- Mobile Scanner Session: a legacy short-lived WebRTC connection between a phone capture client and a Chrome target. Neither the current full app nor the current App Clip uses this delivery path.
- Scanner Work Session: the legacy WebRTC work period used for session-based access accounting. Retained for older clients; it does not govern current local iPhone capture or App Clip cloud delivery.
...
- Paired Mode Switch: a legacy transport term for changing capture modes without creating a new WebRTC session.
...
- Cloud workspace and mobile capture sync require the Pro capability, granted by a verified StoreKit or complimentary entitlement. Local iPhone capture and history do not require Pro. StoreKit entitlement is tied to the account's stable `appAccountToken`; client claims alone never unlock Pro.
- The App Clip stays free and temporary. Its QR grants short-lived access to the signed-in Chrome account's workspace. Captures use cloud delivery, and the Clip can select an online computer for cursor insertion. It never holds an account Device Credential or Clerk session.
