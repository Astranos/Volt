...
test("renders the selected-text search actions and copy control in order", () => {
  ...
  const html = renderToStaticMarkup(<SelectionSuggestionPill actions={["ebay", "google-upc", "pricecharting"]} actionWidths={[114, 135, 125]} onCopy={vi.fn()} onAction={vi.fn()} position={position} />);
  ...
  expect(html).toContain("grid-template-columns:minmax(0, 114fr) minmax(0, 135fr) minmax(0, 125fr)");
});
...
test("renders a custom action order before the fixed copy control", () => {
  ...
  const html = renderToStaticMarkup(<SelectionSuggestionPill actions={["ask-gemini", "google-search"]} actionWidths={[111, 131]} onCopy={vi.fn()} onAction={vi.fn()} position={position} />);
  const labels = [...html.matchAll(/<button[^>]*aria-label="([^"]+)"/g)].map((match) => match[1]);
